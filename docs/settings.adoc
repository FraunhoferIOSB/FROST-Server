...
auth.provider:: The java class used to configure authentication/authorisation. Currently implemented are:
  `de.fraunhofer.iosb.ilt.frostserver.auth.basic.BasicAuthProvider`::: An authentication provider that uses basic authentication for HTTP, and stores user information in a database.
...
Only one of the two parameters `keycloakConfigFile` and `keycloakConfig` needs to be supplied. If both are supplied, the parameter
`keycloakConfig` is used.

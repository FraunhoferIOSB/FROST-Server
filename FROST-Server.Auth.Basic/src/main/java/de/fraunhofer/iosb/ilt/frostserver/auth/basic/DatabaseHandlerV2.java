 ...
 */
public class DatabaseHandlerV2 extends DatabaseHandler {

    public static final String LIQUIBASE_CHANGELOG_FILENAME = "liquibase/basicAuthTablesV2.xml";

    private static final Logger LOGGER = LoggerFactory.getLogger(DatabaseHandlerV2.class);

    protected DatabaseHandlerV2(CoreSettings coreSettings) {
        super(LIQUIBASE_CHANGELOG_FILENAME, coreSettings);
    }

    @Override
    protected Condition passwordCondition(String passwordOrHash) {
        return TableV2Users.USERS.userPass.eq(plainTextPassword
                ? DSL.val(passwordOrHash)
                : DSL.function(
                        "crypt", String.class, DSL.val(passwordOrHash), TableV2Users.USERS.userPass));
    }
    ...
    public boolean isValidUser(UserData userData) {
        ...
        try (final ConnectionWrapper connectionProvider = new ConnectionWrapper(authSettings, connectionUrl)) {
            ...
            Result<Record1<String>> roles = dslContext
                    .select(TableV2UsersRoles.USER_ROLES.roleName)
                    .from(TableV2Users.USERS)
                    .leftJoin(TableV2UsersRoles.USER_ROLES).on(TableV2Users.USERS.userName.eq(TableV2UsersRoles.USER_ROLES.userName))
                    .where(
                            TableV2Users.USERS.userName.eq(userData.userName)
                                    .and(passwordCondition(userData.userPass)))
                    .fetch();
            roles.getValues(TableV2UsersRoles.USER_ROLES.roleName)
                    .stream()
    ...
    public boolean userHasRole(String userName, String userPassOrHash, String roleName) {
        ...
        try (final ConnectionWrapper connectionProvider = new ConnectionWrapper(authSettings, connectionUrl)) {
            ...
            Record1<Integer> one = dslContext
                    .selectOne()
                    .from(TableV2Users.USERS)
                    .leftJoin(TableV2UsersRoles.USER_ROLES)
                    .on(TableV2Users.USERS.userName.eq(TableV2UsersRoles.USER_ROLES.userName))
                    .where(
                            TableV2Users.USERS.userName.eq(userName)
                                    .and(passwordCondition(userPassOrHash))
                                    .and(TableV2UsersRoles.USER_ROLES.roleName.eq(roleName)))
                    .fetchOne();
            return one != null;
        } catch (SQLException | RuntimeException exc) {
    ...
    public boolean userHasRole(String userName, String roleName) {
        ...
        try (final ConnectionWrapper connectionProvider = new ConnectionWrapper(authSettings, connectionUrl)) {
            ...
            Record1<Integer> one = dslContext
                    .selectOne()
                    .from(TableV2UsersRoles.USER_ROLES)
                    .where(
                            TableV2UsersRoles.USER_ROLES.userName.eq(userName)
                                    .and(TableV2UsersRoles.USER_ROLES.roleName.eq(roleName)))
                    .fetchOne();
            return one != null;
        } catch (SQLException | RuntimeException exc) {

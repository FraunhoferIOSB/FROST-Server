...
|`frost.http.ingress.rewriteTraget`          | Value for the `rewriteAnnotation`. Path, which is used to access the FROST-HTTP service                                                                                                                                                               | `/FROST-Server`                                                                           |
...
Since the HTTP endpoint of FROST is reachable under the `/FROST-Server`-path, we leverage the Ingress rewriting capability. **Caution: Our configuration is specific for nginx ingress controller. It needs to be adjusted, if another ingress controller is used.**
...
import de.fraunhofer.iosb.ilt.frostserver.persistence.postgres.relationalpaths.AbstractQMultiDatastreamsObsProperties;
...
public class PathSqlBuilderImp<I extends ComparableExpressionBase<J> & Path<J>, J extends Comparable> implements PathSqlBuilder {
    ...
    private void addOrderAndFilter(Query query, PersistenceSettings settings) {
        if (query != null) {
            ...
            isFilter = true;
            de.fraunhofer.iosb.ilt.frostserver.query.expression.Expression filter = query.getFilter();
            if (filter != null) {

...
import de.fraunhofer.iosb.ilt.frostserver.util.wktparser.nodes.WktMultiPolygon;
...
/**
 * Parses the AST generated by CongoCC.
 */
public class WktParser extends Visitor {
    ...
    private List<List<LngLatAlt>> lastLinearRingList;
    private Polygon lastPolygon;
    private MultiPolygon lastMultiPolygon;
    ...
    public void visit(WktPolygon node) {
        ...
        List<LinearRingList> children = node.childrenOfType(LinearRingList.class);
        lastLinearRingList = null;
        visit(children.get(0));
        lastPolygon = polygonFromLinearRingList(lastLinearRingList);
        result = lastPolygon;
    }

    private Polygon polygonFromLinearRingList(List<List<LngLatAlt>> linearRingList) {
        Polygon polygon = new Polygon();
        polygon.setExteriorRing(linearRingList.get(0));
        int childCount = linearRingList.size();
        for (int i = 1; i < childCount; i++) {
            List<LngLatAlt> linearRing = linearRingList.get(i);
            polygon.addInteriorRing(linearRing);
        }
        return polygon;
    }

    public void visit(WktMultiPolygon node) {
        if (lastMultiPolygon != null) {
            throw new IllegalArgumentException("Previously parsed Polygon not used.");
        }
        List<LinearRingList> children = node.childrenOfType(LinearRingList.class);
        lastMultiPolygon = new MultiPolygon();
        for (LinearRingList child : children) {
            lastLinearRingList = null;
            visit(child);
            if (lastLinearRingList != null) {
                lastMultiPolygon.add(polygonFromLinearRingList(lastLinearRingList));
            }
        }
        result = lastMultiPolygon;
    }

    public void visit(LinearRingList node) {
        if (lastLinearRingList != null) {
            throw new IllegalArgumentException("Previously parsed LinearRingList not used.");
        }
        lastLinearRingList = new ArrayList<>();
        for (Node child : node.children()) {
            lastLinearRing = null;
            visit(child);
            if (lastLinearRing != null) {
                lastLinearRingList.add(lastLinearRing);
            }
        }
    }
    ...
    public void visit(LinearRing node) {
        if (lastLinearRing != null) {
            throw new IllegalArgumentException("Previously parsed LinearRing not used.");
        }

...
public class ExpressionParser extends Visitor {
    ...
    public void handleFunction(Node node) {
        ...
        String name = node.getFirstChild().getImage().toLowerCase(Locale.ROOT);
        if (name.endsWith("(")) {
    ...
    public void visit(P_ComparativeExpression node) {
        ...
        String operator = ((Token) node.get(1)).toString();
    ...
    private Function getFunction(String name) {
        return fr.getFunction(name, admin).setContext(context);
    }

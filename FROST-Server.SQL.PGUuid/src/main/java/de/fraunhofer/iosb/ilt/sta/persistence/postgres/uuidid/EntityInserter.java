...
public class EntityInserter {
    ...
    public EntityChangedMessage updateMultiDatastream(MultiDatastream d, UUID dsId) throws NoSuchEntityException, IncompleteEntityException {
        ...
        MultiDatastream original = (MultiDatastream) pm.get(EntityType.MULTIDATASTREAM, new UuidId(dsId));
    ...
    public boolean insertHistoricalLocation(HistoricalLocation h) throws NoSuchEntityException, IncompleteEntityException {
        Thing t = h.getThing();
        entityExistsOrCreate(t);
        UUID thingId = (UUID) h.getThing().getId().getValue();
    ...
    private static <T extends StoreClause> void insertUserDefinedId(T clause, Path idPath, Entity entity) throws IncompleteEntityException {
        ...
        if (idhandler.useClientSuppliedId()) {
            ...
            clause.set(idPath, (UUID) idhandler.getIdValue());
        }
    }
